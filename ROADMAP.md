...
**`main` vs this branch (resolved 2026-07-08, then reconciled same day):** PR #7 "Phase 16 fantasy UI" merged into `main`, then was reverted almost immediately on the line that became this branch, which built an independent redesign (Stream 2) + the B2B epic (Stream 6) instead. `main` kept building on the *reverted* architecture independently (still received "Phase 16 Sprint C" commits through 2026-07-07) and had no bearing on this work.

**`main` replaced (2026-07-08):** Per explicit user decision, `main`'s old Phase-16 content was archived (preserved in full at branch `archive/phase-16-main-2026-07-08`, nothing deleted) and `main` was force-updated to point at `feat/school-b2b-usage-analytics`'s tip (`8609297`, includes the landing app work from PR #17). `main` now **is** this branch — the "two lines" problem is gone; going forward there is one canonical line. A live Vercel deployment tracks `main` (no public users yet, per the user) — this push will have triggered a redeploy; **verify the deploy succeeds and check prod env vars**, since `main`'s previous deploy was built against the old Phase-16 codebase (different route structure, different env needs) and several env-var gaps were already flagged unresolved in Stream 3 (STRIPE-001, DEPLOY-001) before this switch.
