...
| `STRIPE_PRICE_ID_SCHOOL_SEAT` | Yes (prod) | — | Stripe recurring per-seat Price ID for B2B school subscriptions (price_...). Quantity = a school's `seatLimit`. Reuses `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET`. |
...
STRIPE_PRICE_ID_SCHOOL_SEAT=price_...
